 ...
 * setup.sh last delivered — no sidecar state file, no install log, nothing to keep in
 ...
 * setup.sh skips it and says so, rather than picking for the consumer.
 *
 * Prints one `status<TAB>atom` line per resolved atom, for setup.sh to read:
...
for (const atom of atoms) {
  ...
  if (!fs.existsSync(localManifest)) {
    ...
    console.log(`${identical ? 'clean' : 'unknown'}\t${atom}`);
    continue;
  }
  ...
  console.log(`${local === delivered ? 'clean' : 'modified'}\t${atom}`);
}
