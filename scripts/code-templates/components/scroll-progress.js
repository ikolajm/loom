// ScrollProgress — scroll-linked progress bar (reading indicator).
//
// The structural odd-one-out of the motion core: NOT visibility-triggered, it binds continuously to
// scroll position. A passive, rAF-throttled scroll listener writes progress straight to a CSS var
// (--progress) that drives transform: scaleX() on the fill — NO per-frame setState (a 60fps
// re-render storm is the obvious wrong move; the direct ref-write is the honest path).
//
// Tracks the page by default, or a scroll container via `target`. prefers-reduced-motion is
// intentionally NOT handled — the bar is direct manipulation (it reflects where you actually are),
// not autonomous motion, so suppressing it would just break the indicator.
function generateScrollProgress(name, config, meta) {
  const def = config.default || {};
  const dPosition = def.position || 'top';
  const dShowTrack = def.showTrack ?? false;
...
export const ScrollProgress = forwardRef<HTMLDivElement, ScrollProgressProps>(function ScrollProgress(
  { target, position = '${dPosition}', showTrack = ${dShowTrack}, className, ...props },
  forwardedRef,
) {
...
module.exports = { generateScrollProgress };
