...
function generateSearchBar(name, config, meta) {
  ...
  for (const [tier, sz] of Object.entries(sizes)) {
    const h = sz.height && sz.height.startsWith('height/') ? sz.height.replace('height/', '') : null;
    heightEntries[tier] = h ? `h-${h} text-input-${tier}` : `text-input-${tier}`;
  }
...
${Object.entries(heightEntries).map(([k, v]) => `  ${k}: '${v}',`).join('\n')}
};
...
const SearchBar = forwardRef<HTMLInputElement, SearchBarProps>(
  ({ value, onValueChange, size = '${defaultSize}', onClear, icon, placeholder = 'Search...', disabled, className, ...props }, ref) => {
...
module.exports = { generateSearchBar };
