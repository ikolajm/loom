...
function generateFileUpload(name, config, meta) {
  // Single consumer variant (default). `dragover` is an internal runtime state, applied
  // conditionally — not a cva variant axis (so the manifest doesn't advertise it as a choice).
  const defaultColors = config.variants ? buildVariantStyles(config.variants).default : '';
  const dragoverColors = config.dragover ? buildVariantStyles({ dragover: config.dragover }).dragover : '';
...
const FileUpload = forwardRef<HTMLDivElement, FileUploadProps>(
  ({ size = '${defaultSize}', accept, multiple, onFilesSelected, disabled, error, selectedFile, className, children, ...props }, ref) => {
    ...
    if (children) {
      ...
      content = (
        <>
          <Glyph className={cn(STATUS_TONE[st], st === 'uploading' && 'animate-spin', iconSizeMap[size || '${defaultSize}'])} />
