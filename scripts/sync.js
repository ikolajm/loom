...
// The stylesheet banner, and the one clause in it that changes meaning on delivery.
const STYLESHEET_BANNER = ' — generated, do not edit.';
const MAINTAINER_ADVICE = 'Regenerate from spec/config/.';
const CONSUMER_ADVICE = 'Regenerate with: npm run loom:sync';

/**
 * Stamp the delivering Loom into a generated header.
 *
 * Both headers are single-source — `BANNER` in generate-components.js and one
 * template in generate-tokens-css.js — so each is matched on its own phrase
 * rather than on a shared one. A file carrying neither throws, because an edit
 * that matched nothing reports exactly like one that worked, and this whole
 * change exists because a header said nothing while looking like it said
 * something.
 *
 * @param {string} text file contents
 * @param {string} prov what `provenance()` returned
 * @returns {string}
 */
function stamp(text, prov) {
  // The bare phrase, not "Generated by Loom —": theme-init.js says "Generated by
  // Loom; do not edit here" and the em-dash form missed it. The stylesheets say
  // neither and get their own rule. Found by the throw below on the first real
  // sync, which is what it is there for.
  const banners = [
    ['Generated by Loom', `Generated by Loom ${prov}`],
    [STYLESHEET_BANNER, ` — generated by Loom ${prov}, do not edit.`],
  ];
  const banner = banners.find(([phrase]) => text.includes(phrase));
  if (!banner) throw new Error('no generated header to stamp');
  const stamped = text.replace(banner[0], banner[1]);

  if (banner[0] !== STYLESHEET_BANNER) return stamped;

  // Advice that is true in Loom and false everywhere it is delivered. A consumer
  // has no spec/config/ — and the nearest one that does is Loom's own, where
  // regenerating emits Loom's brand instead of theirs, which is the failure this
  // file's own header already names. One banner, two audiences; this is the
  // delivery half. Asserted rather than replaced blind, for the same reason the
  // banner itself is: a correction that matched nothing looks like one that worked.
  if (!stamped.includes(MAINTAINER_ADVICE)) {
    throw new Error('stylesheet banner without its regenerate line');
  }
  return stamped.replace(MAINTAINER_ADVICE, CONSUMER_ADVICE);
}

/** Copy a generated file into a consumer, stamped with the Loom delivering it. */
function deliver(from, to, prov) {
  try {
    fs.writeFileSync(to, stamp(fs.readFileSync(from, 'utf8'), prov));
  } catch (error) {
    die([`ERROR: ${path.basename(from)} — ${error.message}.`,
      'Every delivered file is stamped with the Loom that produced it, so one',
      'without a generated header would reach a consumer unattributed. If a',
      "generator's banner changed, stamp() is where to change it too."]);
  }
}
...
function main(argv) {
  ...
  for (const atom of tokensOnly ? [] : [...atoms, 'cn']) {
    ...
    deliver(path.join(CATALOG, file), path.join(dest, file), prov);
...
module.exports = { main, provenance, stamp };
