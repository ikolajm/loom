...
| [fonts](fonts.md) | Two-pipeline font handling — runtime Google Fonts `<link>` (code) vs. `loadFontAsync` (Figma); the GF↔Figma parity gap + substitute-Inter handling |
| [scaffold-playground-patterns](scaffold-playground-patterns.md) | Gotchas for building a story-driven component playground (applies to `catalog-playground/`) |
