  ...
  'flex items-center justify-around w-full',
  {
    variants: {
      ...
      size: {
        ...
        md: 'h-bottom-bar-md gap-1 text-label-md',
        lg: 'h-[72px] gap-1 text-label-lg',
      },
    },
    defaultVariants: {
      variant: 'default',
...
const BottomNavItem = forwardRef<HTMLButtonElement, React.ComponentPropsWithoutRef<'button'> & { active?: boolean; icon?: React.ReactNode; }>(
  ({ active = false, icon, className, children, ...props }, ref) => (
    <button ref={ref} type="button" className={cn(
      'flex flex-1 flex-col items-center justify-center gap-0.5 h-full px-3 interactive cursor-pointer transition-colors control text-on-surface-variant',
      active && 'text-primary',
      className
    )} {...props}>
