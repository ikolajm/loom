...
Vite, Django, Next, static sites. [`docs/gotchas.md`](docs/gotchas.md) states what the
substrate needs from an engine, and nothing about any particular one. Where
nothing can consume a stylesheet at all, `tokens.css` is read as a table of values
instead — [below](#read-the-values-somewhere-loom-cannot-reach).
...
Atoms grouped by category, plus `ThemeProvider`, which is its own kind — each generated as a source file + a `.manifest.json` (its dependency/variant contract). `cn` and `theme-init` are delivered alongside them and are not components. The index is generated to [`catalog/atoms.json`](catalog/atoms.json), grouped by category — read it there rather than from a copy here. It is an inventory, not a pick list: the sync copies every atom, and there is no mechanism for taking a subset.

**The catalog is a worked example, not a component library.** One reference per distinct way of wiring something to the class layer: `button` and `badge` for tone x treatment x `data-size` plus `asChild`, `form-field` for the validity cascade into `.control`, `dialog` for a Radix portal, `select` for a Radix form control. Anything else you need, build — Radix is already the primitive layer, and what Loom uniquely owns is tokens to classes.
...
`strict` and `noUnusedLocals` — the only thing in the repo that reads the emitted `.tsx`
as code rather than as text.

```bash
npm install     # typescript, @types/react, and the packages the atoms import
npm run generate
```
...
Re-run these any time you change a value in `spec/answers.json` or a component schema in `spec/config/components/`. `node scripts/code-templates/orchestrator.js --list` shows the individual code generators (`tokens`, `components`, `preview-html`, `verify`); `--only <target>` runs one.
...
That cuts both ways, so the names Loom occupies are published: [`catalog/classes.json`](catalog/classes.json) lists every class the stylesheets define, generated from the emitted CSS on each run. Reusing one is a deliberate override when you meant it and a quiet merge when you did not — check the list before naming a class of your own.

**The first slot is yours, and Loom never writes to it.** Your reset goes in `@layer loom.reset`, imported before `tokens.css`; left unlayered it outranks every rule Loom ships, silently. That is the install step below, and the failure mode is in [`docs/gotchas.md`](docs/gotchas.md) — which is also where the import order stops being documentation and becomes the mechanism. Don't reorder the imports.
...
*how far apart two things sit given what contains them*, not five arbitrary sizes. Reading
inward:
...
moves the whole set together, so a compact product is one answer rather than five decisions.

**Every category has a `compact` twin** — `--section-compact-gap` beside `--section-gap`.
The set is complete rather than partial, which is why `--content-compact-max-width` carries
the same value as `--content-max-width`: compact is a statement about padding and gap, and
the column a page reads in does not get narrower because the spacing inside it tightened.
Every var has a twin, so switching a context over never needs a list of exceptions.
This is the density mechanism Loom ships that is *not* a build-time answer: point a rule at
the compact value where the context is tight — a dense table, a sidebar, a narrow viewport —
and the hierarchy holds at a smaller step instead of collapsing. Loom does not choose when
that happens and ships no breakpoints to hang it on, because the width at which a layout
gets tight is a property of your layout, not of the substrate. (Custom properties are not
valid in a media query condition, so a breakpoint could not be a token here even if Loom
had an opinion about the value.)
...
**Fonts are named here and loaded by you.** The questionnaire takes one family per role (`heading` / `body`) and `tokens.css` emits them as `'Your Family', system-ui, sans-serif`. **Loom recommends no typeface** — that is identity, and identity is yours; what Loom supplies is the ramp under it, six roles across two families ([the table](spec/questionnaire.md#fonts--heading--body)). Loading the bytes is yours too: a `<link>`, an `@font-face`, `next/font`, an `@fontsource` package, whatever your framework prefers. How to spell the family, which weights the ramp asks for and what happens on each surface when you get it wrong are all in that table's section; the short version is that every way of getting it wrong fails silently.
...
`npm run figma` writes `generated/figma-scripts/` — `00_shared-utils.js` (global helpers) plus the numbered step scripts. `npm run generate` prints how many the assembler emitted, and [`docs/pipeline.md`](docs/pipeline.md) has how they are put together. They build **variables, text styles, effect styles and the page layout** — the token half. Figma does not receive components: it has no notion of a class, so the class layer has no representation there, and a Figma component was only ever a snapshot of one combination rather than the rule that generates it. Build the components you need from the variables. To build the Figma file:
...
3. Paste the step scripts **in numeric order**. Re-running one later only needs its own step script re-pasted.
...
[`docs/pipeline.md`](docs/pipeline.md) traces the derivation chain end to end — the three commands, how a config file is resolved, what each generator transforms rather than copies, and where a symptom points. Start there for either job.
